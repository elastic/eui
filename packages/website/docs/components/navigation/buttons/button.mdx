...
:::warning CSS selector `:disabled` does not apply
:::
...
:::warning Default `disabled` test matchers fail
:::
...
1. **Jest DOM**: Use the `.toBeEuiDisabled()` instead of `.toBeDisabled()`
...
2. **Cypress**: Use the `.should('be.euiDisabled')` instead of `.should('be.disabled')`
...
3. **Enzyme (Legacy)**: Use the `.hasEuiDisabledProp()` instead of `.props().disabled`
