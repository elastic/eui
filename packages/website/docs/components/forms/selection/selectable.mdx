...
**EuiSelectable** aims to make the pattern of a selectable list (with or without search) consistent across implementations. It is the same concept used in [**EuiComboBox**](/docs/forms/selection/combo-box/) and [**EuiFilterGroup**](/docs/forms/search-filter/filter-group/). **This is not intended for [primary navigation](/docs/display/list-group)** but can be used to simplify the construction of popover navigational menus; i.e. the spaces menu in the [header](/docs/layout/header). See EUI's <a href="https://github.com/elastic/eui/discussions/7049" target="_blank">in-depth guide on which selection component to choose</a> for more information.
...
## Options can be mixed (indeterminate)[](/docs/forms/selectable#options-can-be-mixed-indeterminate)
...
The width has been made to always be 100% of its container, including stretching the search input. When used inside of [**EuiPopover**](/docs/layout/popover), we recommend setting a width (or min/max values) via CSS on the element containing the list to avoid expansion and contraction. By default, the height is capped at showing up to 7.5 items. It shows half of the last one to help indicate that there are more options to scroll to. To stretch the box to fill its container, pass 'full' to the `height` prop.
...
You can additionally customize individual tooltip behavior by passing `toolTipProps`, which accepts any configuration that [**EuiToolTip**](/docs/display/tooltip) accepts.
