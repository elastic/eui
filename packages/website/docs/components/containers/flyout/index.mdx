...
**EuiFlyout** is a low-level atom that provides a fixed position panel that pops in from the side of the window. It is designed to be a flexible foundation upon which more complex flyout patterns can be built.

:::tip Looking for usage guidance?

For task-oriented recipes like details flyouts, edit/settings forms, and push vs overlay variants, see the [Flyouts pattern](/docs/patterns/navigation/flyouts).
