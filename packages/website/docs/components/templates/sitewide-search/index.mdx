...
If you want to override the `colorMode` manually, you can pass a `theme` prop to the component. This allows you 
to apply different themes to the search and popover components.

import { Example } from '@site/src/components';

<Example.Snippet>
  ```tsx
  <EuiSelectableTemplateSitewide theme={{ search: 'default', popover: 'inverse' }} />
  ```
</Example.Snippet>
...
export default () => {
  return (
    <EuiSelectableTemplateSitewide
      ...
      theme={{ search: 'default', popover: 'inverse' }}
    />
  );
};
```
