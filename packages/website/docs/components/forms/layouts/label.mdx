...
**EuiFormFieldset** wraps its children in a `<fieldset>` with the option to add a `<legend>` via the `legend` object prop. While most commonly used for [checkboxes](../selection/checkbox-and-checkbox-group.mdx#legends) and [radios](../selection/radio-and-radio-group.mdx#legends), it can also be used to group related inputs and selects under a shared label.
