...
When using sorting with **EuiBasicTable**, make sure each row has a unique identifier. Duplicate identifiers can cause sorting to display incorrect results, such as showing fewer items than expected. Ensure your data has unique `id` values.
:::
