...
Do not use an error warning when the error is directly related to the current task. Instead, use the [error page pattern](./error_pages.mdx) or the [error banner pattern](error_banners.mdx#error-banner).
...
Use the [EuiToast](../../components/display/toast/overview.mdx#danger) component, with the props:
- `color="danger"`
- `iconType="alert"`
