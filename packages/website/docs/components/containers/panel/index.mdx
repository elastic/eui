...
Panels default to a flat bordered look (`hasBorder=true`, `hasShadow=false`). Use shadow only when you need elevation.
...
The most basic use is to simply add padding around your content. The preferred padding options are `none`, `s`, `m` (default) and `l`.
...
By default, **EuiPanel** is flat with `hasBorder=true` on and `hasShadow=false`. Use `hasShadow` when you need elevation. Be mindful of not using too many [nested panels](#guidelines) with these settings.
