...
When a data cell is focused and no text is selected, **Ctrl/Cmd+C** copies the cell's visible text. If you have selected text, that selection is copied instead. Control columns are skipped.

This copies the rendered text, not a raw data value.
...
const cellActions5: EuiDataGridColumnCellAction[] = [
  ...cellActions3,
  ({ Component }) => (
    <Component onClick={() => {}} iconType="star">
      Custom action 1
    </Component>
  ),
  ({ Component }) => (
