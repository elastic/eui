...
EUI provides the `--euiFlyoutOffsetInlineEnd` CSS variable to allow consumers to declare viewport constraints that affect flyout positioning. This is useful when your application has fixed UI elements (like sidebars or navigation panels) that reduce the effective viewport width.

#### Usage

Set the CSS variable at the `:root` level to apply the offset to all flyouts:

```css
:root {
  --euiFlyoutOffsetInlineEnd: 300px;
}
```

Or apply it dynamically via JavaScript:

```javascript
document.documentElement.style.setProperty('--euiFlyoutOffsetInlineEnd', '300px');
```
