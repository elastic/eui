...
EUI icons use explicit dynamic imports to load SVGs on demand. Each icon in `EuiIcon`'s internal mapping is a separate chunk that your bundler generates and loads lazily at runtime:
