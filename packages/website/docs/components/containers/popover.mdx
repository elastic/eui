...
For placement, behavior, accessibility, and alternatives, see the [Popovers pattern](../../patterns/navigation/popovers.mdx).

:::

Use **EuiPopover** to reveal lightweight, contextual content on demand. Keep tasks short and focused.

## Accessibility (atom-level)

- The trigger must be keyboard-focusable
- Open/close should be controlled by click (not hover/focus)
- Popovers auto-close on outside click by default
