...
To customize the global theme of your app, use the `theme`, `colorMode`, and `modify` props (documented in [EuiThemeProvider](../theming/theme-provider.mdx)). For instance, it's likely that you will want to implement color mode switching at the top level:
