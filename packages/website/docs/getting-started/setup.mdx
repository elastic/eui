...
<EuiTable>
  ...
  <EuiTableBody>
    ...
    <EuiTableRow>
      <EuiTableRowCell>Vite/Rollup/Parcel/esbuild</EuiTableRowCell>
      <EuiTableRowCell align="center">⚠️</EuiTableRowCell>
      <EuiTableRowCell>
        Supported with caveats (<Link href="https://github.com/elastic/eui/issues/7630">details</Link>)
      </EuiTableRowCell>
    </EuiTableRow>
    <EuiTableRow>
      <EuiTableRowCell>ES modules</EuiTableRowCell>
      <EuiTableRowCell align="center">✅</EuiTableRowCell>
      <EuiTableRowCell>Fully supported (required)</EuiTableRowCell>
    </EuiTableRow>
    <EuiTableRow>
      <EuiTableRowCell>CommonJS</EuiTableRowCell>
      <EuiTableRowCell align="center">❌</EuiTableRowCell>
      <EuiTableRowCell>
        Runtime support ended after EUI v117.x. The Jest-only <code>test-env</code> build remains CommonJS.
      </EuiTableRowCell>
    </EuiTableRow>
    <EuiTableRow>
...
You can import React components and services from the top-level EUI module.
...
Try avoiding direct imports from the build directories. They may break when EUI is updated.

Test utilities are published from `test-env/test` (Jest) and `es/test` (ESM).

```tsx
import { findTestSubject } from '@elastic/eui/test-env/test'; // Enzyme (Jest)
import { findByTestSubject, render, screen } from '@elastic/eui/test-env/test/rtl'; // React Testing Library (Jest)
```
