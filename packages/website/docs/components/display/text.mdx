...
Using the `size` prop on **EuiText** you can scale text relative to the default (`s`, ~14px — aligned with the unstyled body/html base). This demo compares the scaling for all sizes. The goal is that every line-height lands on the `4px` baseline grid.
