...
For panel sets, collapsible layouts, controlled sizing, and responsive patterns, see the [Resizable panels pattern](../patterns/layouts/resizable-panels.mdx).

:::

Use **EuiResizableContainer** to create split views where users can adjust panel sizes. It provides render-prop children to compose `EuiResizablePanel` and `EuiResizableButton`.

## Accessibility (atom-level)

- Make scrollable panels focusable with `tabIndex={0}`
- Resizer supports keyboard resizing (Left/Right or Up/Down)
