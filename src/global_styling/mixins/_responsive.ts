...
export type EuiBreakpointKeys = keyof typeof BREAKPOINT_KEYS;
...
export const euiBreakpoint = (sizes: EuiBreakpointKeys[]) => {
  ...
  if (sizes.length === 1 && sizes[0] === 'xs') {
    return `@media only screen and (max-width: ${BREAKPOINT_KEYS[sizes[0]]}px)`;
  } // If sizes only contains one elements and it is the largest size (xl), don't set a max-width
  else if (sizes.length === 1 && sizes[0] === 'xl') {
    return `@media only screen and (min-width: ${BREAKPOINT_KEYS[sizes[0]]}px)`;
    ...
    return `@media only screen and (min-width: ${
      BREAKPOINT_KEYS[minSize]
    }px) and (max-width: ${BREAKPOINT_KEYS[maxSize] - 1}px)`;
  }
};
