...
By default, EUI Cypress tests are run using the latest supported version of React (18).
You can change that behavior by passing `--react-version` set to `17` or `18` (peer range). 
...
EUI now has the ability to record failed Cypress tests as Buildkite CI artifacts. This feature is turned off by default, but can be turned on by changing `video: false` to `video: true` in [`packages/eui/cypress.config.ts`](https://github.com/elastic/eui/blob/main/packages/eui/cypress.config.ts). You can verify videos are being recorded by changing a Cypress test to fail on purpose, then running `yarn test-cypress` locally. Video files will be stored in `packages/eui/cypress/videos/`.
...
### Cypress Axe
EUI components are tested for accessibility on every pull request (Buildkite Cypress a11y job) and locally via `yarn test-cypress-a11y`. This allows us to test changes to the DOM such as accordions being opened, or modal dialogs being triggered, more comprehensively. We use [cypress-axe](https://github.com/component-driven/cypress-axe) to access the underlying axe-core API methods and rulesets.
