...
The **EuiListGroup** component is used to present **EuiListGroupItems** in a neatly formatted list. Use `bordered` properties for bordered presentations.
...
export default () => {
  ...
  return (
    <Fragment>
      ...
      <EuiListGroup bordered={showBorder}>
...
export default () => {
  return <EuiListGroup listItems={myContent} color="primary" />;
};
```
...
export default () => (
  <>
    <EuiListGroup>
      <EuiListGroupItem href="#" label="Text by default (xs)" />
      ...
      <EuiListGroupItem href="#" label="Subdued (l)" color="subdued" />
    </EuiListGroup>
