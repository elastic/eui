...
This modal example registers a listener on the `keydown` event and listens for ESC key presses, which closes the open modal. The `event` prop accepts any
[`WindowEventMap`](https://developer.mozilla.org/en-US/docs/Web/API/WindowEventMap) event name.
