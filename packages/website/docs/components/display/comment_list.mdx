...
6.  `actions`: Custom actions that the user can perform from the comment's header. When having multiple actions, consider grouping them in a nested menu system using a [EuiPopover](../containers/popover.mdx) with a [EuiContextMenu](../navigation/context_menu.mdx).
