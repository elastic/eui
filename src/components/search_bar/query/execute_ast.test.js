...
describe('execute ast', () => {
  test('single matching field clause', () => {
    ...
    const result = executeAst(AST.create([AST.Field.must('name', 'john')]), items);
  ...
  test('single matching field clause with negation', () => {
    ...
    const result = executeAst(AST.create([AST.Field.mustNot('name', 'john')]), items);
    expect(result).toHaveLength(1);
  ...
  test('single matching multi-valued field clause', () => {
    const items = [{ name: 'john' }, { name: 'joe' }];
    let result = executeAst(AST.create([AST.Field.must('name', ['john', 'doe'])]), items);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('john');

    result = executeAst(AST.create([AST.Field.must('name', ['joe', 'doe'])]), items);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('joe');

    result = executeAst(AST.create([AST.Field.must('name', ['foo', 'bar'])]), items);
    expect(result).toHaveLength(0);
  });

  test('single non-matching field clause', () => {
    const items = [{ name: 'john' }, { name: 'joe' }];
    const result = executeAst(AST.create([AST.Field.must('name', 'foo')]), items);
    expect(result).toHaveLength(0);
  });
  ...
  test('multiple field clause - non-matching', () => {
    ...
    const result = executeAst(
      AST.create([AST.Field.must('name', 'john'), AST.Field.must('name', 'joe')]),
      items
    );
    expect(result).toHaveLength(0);
  });
  ...
  test('multiple field clause - non-matching 2', () => {
    ...
    const result = executeAst(
      AST.create([AST.Field.must('name', 'foo'), AST.Field.must('age', '7')]),
      items
    );
    expect(result).toHaveLength(0);
  });
  ...
  test('multiple field clause - matching', () => {
    const items = [{ name: 'john', age: 5 }, { name: 'joe' }];
    const result = executeAst(
      AST.create([AST.Field.must('name', 'john'), AST.Field.must('age', '5')]),
      items
    );
    expect(result).toHaveLength(1);
  });
  ...
  test('term clauses - no default fields specified', () => {
    const items = [{ name: 'john', description: 'doe', age: 5 }, { name: 'joe' }];
  ...
  test('phrases', () => {
    ...
    const result = executeAst(AST.create([AST.Field.must('name', 'John Doe')]), items);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('john doe');
  });
  ...
  test('or fields', () => {
    ...
    const result = executeAst(AST.create([AST.Field.must('name', ['john', 'bar'])]), items);
