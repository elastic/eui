...
:::warning EuiButtonIcon requires an aria-label to express the meaning to screen readers.
...
:::warning Do not add aria-pressed or isSelected if you also change the readable text.
...
:::warning In order for groups to be properly read as groups with a title, the legend prop is required. This is only for accessibility, however, so it will be visibly hidden.
