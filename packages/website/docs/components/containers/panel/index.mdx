...
Panels default to a flat bordered look (`hasBorder`, no shadow) with `border.radius.panel` corners. Use shadow only when you need elevation.
...
The preferred padding set is `none`, `s`, `m` (default), and `l`. Prefer these over `xs` / `xl`, which remain available but are soft-deprecated on **EuiPanel**.
...
By default, **EuiPanel** is flat: `hasBorder` on and `hasShadow` off. Use `hasShadow` when you need elevation (incompatible with border). Just be sure not to include too many [nested panels](#guidelines) with these settings.
...
export default () => {
  return (
    <div>
      <EuiPanel>
        <EuiCode>Default (hasBorder, no shadow)</EuiCode>
      </EuiPanel>

      <EuiSpacer />

      <EuiPanel hasBorder={false} hasShadow>
        <EuiCode>{'hasBorder={false} hasShadow'}</EuiCode>
      </EuiPanel>
    </div>
  );
};
