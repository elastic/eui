...
`EuiButton` uses a full **pill** radius. `EuiButtonEmpty` and `EuiButtonIcon` use the theme **control** radius (`border.radius.control`, ~8px) so they align with form controls.
