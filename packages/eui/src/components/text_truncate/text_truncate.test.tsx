...
describe('EuiTextTruncate', () => {
  ...
  describe('tooltip', () => {
    ...
    it('makes the content div keyboard focusable when truncating', () => {
      const { container } = render(
        <EuiTextTruncate text="Hello world" width={0} />
      );
      expect(container.querySelector('.euiTextTruncate')).toHaveAttribute(
        'tabindex',
        '0'
      );
    });

    it('does not make the content div keyboard focusable when not truncating', () => {
      const { container } = render(
        <EuiTextTruncate text="Hello world" width={50} />
      );
      expect(container.querySelector('.euiTextTruncate')).not.toHaveAttribute(
        'tabindex'
      );
    });
  });
