...
`EuiCallout` predefines these semantic meanings with specific colors and icon combinations controlled by the `type` prop.
...
export default () => (
  <EuiCallOut
    ...
    type="warning"
    actionProps={{ primary: { children: 'Link button', href: '#' } }}
  />
);
```
...
*   Avoid using `children` for body text. Use `text` instead.
