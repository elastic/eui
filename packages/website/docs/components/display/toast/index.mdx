...
To show a countdown animation on top of the toast to indicate the remaining time until it dismisses, use `animationMs`.

<Demo previewWrapper={previewWrapper}>
  ```tsx interactive
  import React, { useEffect, useState } from 'react';
  import { EuiToast, EuiButton, EuiFlexGroup } from '@elastic/eui';

  const ANIMATION_MS = 6000;

  export default () => {
    const [animationMs, setAnimationMs] = useState<number | undefined>(undefined);

    useEffect(() => {
      setAnimationMs(ANIMATION_MS);
    }, []);
    ...
    return (
      <EuiFlexGroup direction="column" gutterSize="m">
        <EuiButton
          ...
          onClick={() => setAnimationMs(animationMs != null ? undefined : ANIMATION_MS)}
        >
