...
slug: 
title: Button
description: Guidance for how to select button type, what props to use (color and size), whether to include an icon or not, what labels to use (e.g. Create vs Save), and when
sidebar_position: 1
...
By default, `EuiButton` has the `fill` property set to `false`. These buttons are typically used for **secondary actions**, striking a balance between visibility and visual noise. They are less prominent than primary buttons and can be used alongside them. **Primary actions (CTAs)** should be represented by filled buttons (`fill: true`). To preserve their strong visual emphasis, **only one primary action button** should be visible per view (see more on combining button variants [below](#combining-button-variants)).
...
<EuiFlexGroup gutterSize="m">
  <Guideline type="do" text={` The arrow lets users know they still have more items to fill out. The check icon helps users understand that this is the final action.`}>
  ...
  <Guideline type="dont" text="Unnecessary icons often just distract. This is especially true when the icon is positioned on the right and is hard to interpret.">
...
Empty buttons are for **complementary (tertiary)** actions like close, cancel, filter, and refresh, which typically affect data presentation rather than modification. They reduce a button's visual importance and support `xs` [size](#size) and the [flush](#width-and-flush-property) property for alignment.
...
EUI provides properties to manage color, size, and state to cover various use cases, ensuring consistent interactions.
...
### Width and `flush` property

Button width is generally **determined by content** (label, icon, padding). Icon-only buttons are smaller and **square**. EUI sets a **default minimum width** for consistency, which should be overridden rarely. Use `fullWidth` buttons primarily in narrow forms or for mobile responsiveness; use sparingly in larger desktop viewports.

The `flush` property is available for `EuiButtonEmpty`. It removes padding, ensuring stricter alignment with surrounding content, especially at container edges where empty buttons might otherwise create a ragged line.
...
If the button's readable text (children or `aria-label`) changes, no additional accessibility concern exists.
...
The save button should be the **most dominant** action, consistent across contexts. Use the `fill` property and `primary` color variant, including a noun in the label (e.g., "Save dashboard").

**Consider if an icon is necessary**. Save buttons should be visually dominant, making an icon less necessary compared to other actions. Rely solely on the label to avoid inconsistency.

Exceptions exist, such as the `EuiInlineEdit` component, which offers a save action without a label.
