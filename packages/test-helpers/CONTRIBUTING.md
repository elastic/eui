...
1. **Selectors** — add `data-test-subj` constants to `src/components/<name>/selectors.ts` and re-export the object from `src/selectors.ts`. Never inline test-subj strings anywhere else.
