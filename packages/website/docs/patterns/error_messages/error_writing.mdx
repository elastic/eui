...
A good error message can be the difference between success and failure. We should always try to [help users avoid errors](./help_users_avoid_errors.mdx), but things can always go wrong. Good error messages reassure the user and help them recover quickly.
