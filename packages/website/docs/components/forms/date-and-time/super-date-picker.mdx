...
Time zone information can be made visible in the date picker popovers. To do that, pass the time zone name in the `timeZoneDisplay` prop e.g. `timeZoneDisplay="America/Los_Angeles"`.

Additionally, custom content can be added to the time zone information by leveraging the `timeZoneCustomDisplayRender` render function prop.
  ...
  return (
    <>
      <EuiSuperDatePicker
        ...
        onTimeChange={onTimeChange}
        showUpdateButton={false}
        timeZoneDisplay={TIME_ZONE_NAME}
