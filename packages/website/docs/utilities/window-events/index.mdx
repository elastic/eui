...
This modal example registers a listener on the `keydown` event and listens for ESC key presses, which closes the open modal. The `event` prop accepts any event name from TypeScript's [`WindowEventMap`](https://github.com/microsoft/TypeScript/blob/v5.8.3/src/lib/dom.generated.d.ts#L26880) definition.
