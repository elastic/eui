...
import { IconSearch } from '@site/src/components/display/icons';
import {
  typeToPathMapDocsGlyphIconTypes as iconTypes,
  typeToPathMapSynonyms as iconSynonyms,
} from '../../../../../eui/src/components/icon/icon_map';
...
>
  ...
  export default () => (
    <div css={css`height: fit-content;`}>
      ...
      <EuiSpacer />
      <IconSearch iconTypes={iconTypes} iconSynonyms={iconSynonyms} />
    </div>
  );
  ```
</Demo>
...
import { typeToPathMapDocsLogoIconTypes as iconTypesLogos } from '../../../../../eui/src/components/icon/icon_map';

<Demo scope={{ iconTypes: iconTypesLogos, IconSearch }} previewPadding="none">
  ```tsx
  import React, { useState } from 'react';
  import { css } from '@emotion/react';
  import { EuiButtonGroup, EuiSplitPanel, EuiCodeBlock, EuiSpacer, EuiIcon } from '@elastic/eui';
  import { IconSearch } from '@site/src/components/display/icons';
  import { typeToPathMapDocsLogoIconTypes as iconTypes } from '../../../../../eui/src/components/icon/icon_map';
    ...
    return (
      <EuiSplitPanel.Outer hasBorder={false} hasShadow={false}>
        ...
        <EuiSplitPanel.Inner color="transparent">
          ...
          <IconSearch
            iconTypes={iconTypes}
            placeholder="Search Elastic logos"
            iconSize="xl"
            resultThemeMode={iconColor === 'ghost' ? 'DARK' : undefined}
            renderIcon={(iconType) => (
              <EuiIcon
...
import { typeToPathMapDocsAppIconTypes as iconTypesApps } from '../../../../../eui/src/components/icon/icon_map';

<Demo scope={{ iconTypes: iconTypesApps, IconSearch }}>
  ```tsx
  import React from 'react';
  import { css } from '@emotion/react';
  import { EuiCodeBlock, EuiSpacer } from '@elastic/eui';
  import { IconSearch } from '@site/src/components/display/icons';
  import { typeToPathMapDocsAppIconTypes as iconTypes } from '../../../../../eui/src/components/icon/icon_map';
  ...
  export default () => (
    <div css={css`height: fit-content;`}>
      ...
      <EuiSpacer />
      <IconSearch iconTypes={iconTypes} placeholder="Search app logos" iconSize="xl" />
    </div>
  );
  ```
</Demo>
...
import { typeToPathMapDocsMlIconTypes as iconTypesML } from '../../../../../eui/src/components/icon/icon_map';

<Demo scope={{ iconTypes: iconTypesML, IconSearch }}>
  ```tsx
  import React from 'react';
  import { css } from '@emotion/react';
  import { EuiCodeBlock, EuiSpacer } from '@elastic/eui';
  import { IconSearch } from '@site/src/components/display/icons';
  import { typeToPathMapDocsMlIconTypes as iconTypes } from '../../../../../eui/src/components/icon/icon_map';
  ...
  export default () => (
    <div css={css`height: fit-content;`}>
      ...
      <IconSearch
        iconTypes={iconTypes}
        placeholder="Search machine learning app logos"
        iconSize="xl"
      />
    </div>
  );
...
### Using icons outside of React

EUI publishes raw SVG files and a JSON manifest for consumers who need to use icons outside of the React component layer - for example, to generate canvas images, produce CSS background images at build time, or render icons in non-React environments.

**Individual SVG files** are available under `@elastic/eui/dist/svgs/`. For example:

```
@elastic/eui/dist/svgs/accessibility.svg
@elastic/eui/dist/svgs/tokens/tokenAlias.svg
```

**JSON manifest** (`@elastic/eui/dist/eui-icons.json`) maps icon filename stems to their raw SVG content strings:

```js
import icons from ‘@elastic/eui/dist/eui-icons.json’;
const svg = icons[‘accessibility’];         // plain glyph
const tokenSvg = icons[‘tokens/tokenAlias’]; // token icon
```

:::warning Server-side and build-time use only
`eui-icons.json` is intended for **server-side or build-time use** (e.g. Node.js scripts, SSR, canvas rasterization). Importing it in client-side code will inline all SVG strings into your JavaScript bundle.
:::
...
import { typeToPathMapDocsTokenIconTypes as iconTypesTokens } from '../../../../../eui/src/components/icon/icon_map';

<Demo scope={{ iconTypes: iconTypesTokens, IconSearch }}>
  ```tsx
  import React from 'react';
  import { css } from '@emotion/react';
  import { EuiCodeBlock, EuiSpacer, EuiToken } from '@elastic/eui';
  import { IconSearch } from '@site/src/components/display/icons';
  import { typeToPathMapDocsTokenIconTypes as iconTypes } from '../../../../../eui/src/components/icon/icon_map';
  ...
  export default () => (
    <div css={css`height: fit-content;`}>
      ...
      <IconSearch
        iconTypes={iconTypes}
        placeholder="Search token icons"
        renderIcon={(iconType) => (
          <EuiToken className="eui-alignMiddle" iconType={iconType} />
        )}
      />
    </div>
  );
