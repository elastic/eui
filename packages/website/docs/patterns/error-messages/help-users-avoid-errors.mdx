...
2. sanitize, remove any non-numeric characters, like letters, symbols and whitespace
3. validate the remaining number is greater than 0
   - if it is, goto step 4  
   - if it’s not, highlight an error and tell the user the number must be more more than 0
4. validate the remaining number is less than than 100
