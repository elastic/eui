...
If you need more customization for searching or rendering options, see [EUI's guide on selection components(opens in a new tab or window)](https://github.com/elastic/eui/discussions/7049) .
...
**EuiFilePicker** is a stylized, but generic HTML `<input type="file">` tag. It supports drag and drop as well as on click style selection of files. The example below shows how to grab the files using the [FileList API(opens in a new tab or window)](https://developer.mozilla.org/en-US/docs/Web/API/FileList). Like other form elements, you can wrap it in a **EuiFormRow** to apply a label.
