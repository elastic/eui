...
If the item links to a module that is not GA (beta, lab, etc), you can add a `betaBadgeLabel` and `betaBadgeTooltipContent` to the card and it will properly create and position an [EuiBetaBadge](../../display/badge/index.mdx#beta-badge).
