...
*   **EuiHeaderSectionItemButton**: Specialized button that extends [EuiButtonEmpty](../navigation/buttons/button.mdx#empty) but styled to fit the height of the header with additional `notification` props.
...
export default () => {
  ...
  const sections: EuiHeaderSections[] = [
    {
      items: [<EuiHeaderLogo logoType="horizontal" />],
    },
  ];
