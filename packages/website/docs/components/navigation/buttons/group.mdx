...
Use `variant="selection"` to give buttons toggle behaviour — clicks fire `onChange` with the button's `id`, and the selected state is reflected via `aria-pressed` on each button.
...
export default () => {
  ...
  const [idSelected, setIdSelected] = useState('edit');
