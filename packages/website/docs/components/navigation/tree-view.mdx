...
Each node requires a `label` and a unique `id`. The `icon` prop accepts any [icon or token](../display/icons/index.mdx). You can also specify a different icon for the open state with the `iconWhenExpanded` prop. Use the nodes' `children` prop to nest more nodes.
