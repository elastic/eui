...
Modals come with a wrapping **EuiOverlayMask** to obscure the content beneath, but unlike flyouts, modals cannot be dismissed by clicking on the overlay mask. This is inline with our [modal usage guidelines](#guidelines) which requires there to be a primary action button, even if that button simply closes the modal.
