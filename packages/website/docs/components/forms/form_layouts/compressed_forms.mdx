...
When using compressed, horizontal form styles, it is best not to overload the UI with expansive help text. If it's short and part of the validation, use `helpText`. However, if it's an explanation of the control, consider wraping the label with an [**EuiToolTip**](../../display/tooltip.mdx) and appending the `questionInCircle` icon to it.
...
Always use the compressed version of forms and elements when they exist inside of a [popover](../../layout/popover.mdx).
