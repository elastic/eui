---
slug: /forms/selectable
id: forms_selectable
---

# Selectable

**EuiSelectable** aims to make the pattern of a selectable list (with or without search) consistent across implementations. It is the same concept used in [**EuiComboBox**](/docs/forms/combo-box) and [**EuiFilterGroup**](/docs/forms/filter-group). **This is not intended for [primary navigation](/docs/display/list-group)** but can be used to simplify the construction of popover navigational menus; i.e. the spaces menu in the [header](/docs/layout/header). See [EUI's in-depth guide on which selection component to use (opens in a new tab or window)](https://github.com/elastic/eui/discussions/7049) for more information.
