...
Using the `size` prop on **EuiText** you can scale text relative to the default (`s`, ~14px — aligned with the unstyled body/html base). This demo compares the scaling for all sizes. The goal is that every line-height lands on the `4px` baseline grid. Pass `size="m"` when you need the previous ~16px default.
...
By default, bare **EuiText** (no `color` prop) uses the subdued text color so body copy reads quieter. Pass `color="default"` when you need full paragraph contrast. **EuiTextColor**'s `default` remains the paragraph color and is unchanged.
