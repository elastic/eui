 ...
 */
  ...
  return {
    ...
    // Positions
    top: css`
      animation: ${euiToolTipAnimationVertical(`-${euiTheme.size.base}`)} ${animationTiming};
    `,
    bottom: css`
      animation: ${euiToolTipAnimationVertical(euiTheme.size.base)} ${animationTiming};
    ...
    `,
    left: css`
      animation: ${euiToolTipAnimationHorizontal(`-${euiTheme.size.base}`)} ${animationTiming};
...
export const euiToolTipPopoverStyles = ({ euiTheme, colorMode }: UseEuiTheme) => ({
  ...
  euiToolTip__title: css`
    ...
    ${logicalCSS('border-bottom', `solid ${euiTheme.border.width.thin} ${euiToolTipBorderColor(euiTheme, colorMode)}`)};
