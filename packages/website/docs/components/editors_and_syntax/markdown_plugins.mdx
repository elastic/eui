...
*   `exclude`: an array of default plugins to [unregister](/docs/editors-syntax/markdown-editor#unregistering-plugins)
*   `parsingConfig`: allows overriding the configuration of any default parsing plugin
*   `processingConfig`: currently only accepts a `linkProps` key, which accepts any prop that [EuiLink](/docs/navigation/link) accepts
