...
**EuiTabs** is a navigation and selection atom that renders a focusable, keyboard-navigable tablist with tabs and a selected tab. It provides styling and semantics while remaining unopinionated about routing, data loading, or layout rules.

:::tip Looking for usage guidance?

For when to use tabs vs alternatives, nested tabs, responsive collapse, and secondary actions, see the [Tabs pattern](/docs/patterns/navigation/tabs-usage).

:::

## Purpose

Render a focusable, keyboard-navigable tablist with tabs and a selected tab. Styling + semantics handled; no opinionated routing, data loading, or layout rules.
...
**EuiTabs** allow a `size` prop. The default (medium) size is appropriate for most use cases. The small size is best for when placing inside popovers or other small containers. The large size is for primary page navigation.
...
**EuiTabbedContent** is a convenience wrapper. For new implementations, we recommend using the [Tabs pattern](/docs/patterns/navigation/tabs-usage) which provides more flexibility and better control over tab behavior.
