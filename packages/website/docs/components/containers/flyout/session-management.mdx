...
return () => {
  ...
  return (
    <>
      ...
      {isOpenFlyoutA && (
        <EuiFlyout session size="s" aria-label="Flyout A - main" onClose={() => setIsOpenFlyoutA(false)}>
          <EuiFlyoutHeader>
            <EuiTitle><h2>Welcome to the flyout session management journey!</h2></EuiTitle>
          </EuiFlyoutHeader>
          <EuiFlyoutBody>
            <EuiText>This flyout is rendered with the <EuiCode>session=&#123;true&#125;</EuiCode> prop set which creates a new flyout session and marks this flyout as main.</EuiText>
            <EuiSpacer />
      ...
      {isOpenFlyoutB && (
        <EuiFlyout session size="m" aria-label="Flyout B - main" onClose={() => setIsOpenFlyoutB(false)}>
          <EuiFlyoutHeader>
            <EuiTitle><h2>Glad to meet you, {name || 'stranger'}!</h2></EuiTitle>
          </EuiFlyoutHeader>
          <EuiFlyoutBody>
            <EuiText>
              This flyout is also rendered with the <EuiCode>session=&#123;true&#125;</EuiCode> prop added which creates a <strong>second flyout session</strong> and marks this flyout as main in that session.
              <br /><br />
...
When set to `true`, it activates the underlying flyout session management logic
...
Each time you set `session={true}`, you create a new flyout session and mark
the rendered flyout as [main](#main-flyouts).
To create a [child flyout](#child-flyouts) - a flyout that belongs to the same
session as the main flyout - you simply render it inside the main flyout.
...
with `session={true}` prop creates a new session and becomes the main flyout
...
a [main flyout](#main-flyouts) - a flyout with `session={true}` prop set.

They're meant to display secondary level information like the alert visualization.

Child flyouts accept an optional `childBackgroundShaded` boolean prop that lets you change
