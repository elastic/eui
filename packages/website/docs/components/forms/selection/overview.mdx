...
- When you have a small set of options (\<7) and all options are equally important in terms of visibility, use [EuiRadioGroup](./checkboxes_radios/overview).
- When you have a small set of options (\<12) and only the current selected option has UI importance, reach for [EuiSelect](./select) first when possible, over other more complex selection components.
...
- When you have a small set of options (\<12) _and_ if you need to customize how the options are rendered and selected, use [EuiSuperSelect](./super_select).
...
- When you have a small set of options (\<7), and all options are equally important in terms of visibility, use [EuiCheckboxGroup](./checkboxes_radios/overview).
...
- In general, we recommend using [EuiSelectable](./selectable) for most multi-selection purposes. It renders a searchable, vertically-oriented list with icons next to the option representing selection state.
...
- In general, for multi-select scenarios, we recommend using [EuiComboBox](./combo-box) over [EuiSelectable](./selectable) primarily when selections should be immediately visible and unselected options should not. Selections are rendered as a horizontally-oriented list of pills.
...
| Feature | &emsp;[EuiSelect](./select)&emsp; | &emsp;[EuiSuperSelect](./super_select)&emsp; | &emsp;[EuiSelectable](./selectable)&emsp; | &emsp;[EuiComboBox](./combo_box)&emsp; | 
|:---|:---:|:---:|:---:|:---:|
