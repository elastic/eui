...
Always remember to update the [documentation site](https://eui.elastic.co/) in the `packages/website` folder and [add a changelog](./changelogs.md) in the same PR that contains functional changes. We do this in tandem to prevent our examples from going out of sync with the actual components. In this sense, treat documentation no differently than how you would treat tests.
