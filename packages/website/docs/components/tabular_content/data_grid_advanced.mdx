...
`EuiDataGrid` also exposes several underlying methods from [react-window's `VariableSizeGrid` imperative API(opens in a new tab or window)](https://react-window.vercel.app/#/api/VariableSizeGrid) via its `ref`:
