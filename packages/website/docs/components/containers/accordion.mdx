...
Due to the previously mentioned bare styles, it is recommended to place an [EuiSpacer](../layout/spacer.mdx) between accordion items. Padding within each accordion item can be applied via the `paddingSize` prop.
...
const Rows = () => {
  ...
  const growingAccordionDescriptionId = useGeneratedHtmlId({
    prefix: 'growingAccordionDescription',
  });
  const listId = useGeneratedHtmlId({ prefix: 'list' });
  return (
    <EuiText size="s">
      <EuiScreenReaderOnly>
        <p id={growingAccordionDescriptionId}>
      ...
      <p>
        <EuiButton
          ...
          aria-describedby={growingAccordionDescriptionId}
        >
