...
- You have [Kibana repository](https://github.com/elastic/kibana) forked and cloned:
  ...
  yarn kbn bootstrap
  ```
- EUI and Kibana should be sibling directories:
  ```text
  Projects/
  ├── eui/
  └── kibana/
...
```bash
moon run @kbn/ui-shared-deps-npm:watch-webpack
```
...
To watch only a specific package:

```bash
# Watch only @elastic/eui
yarn serve --kibana --package eui
# Shortcuts:
yarn serve -k -p eui

# Watch only @elastic/eui-theme-borealis
yarn serve --kibana --package borealis
# Shortcuts:
yarn serve -k -p borealis

# Watch only @elastic/eui-theme-common
yarn serve --kibana --package common
# Shortcuts:
yarn serve -k -p common
```
...
3. Sync the build artifacts directly into `../kibana/node_modules`.
...
Check the terminal output of the EUI watcher. If the sync succeeded, check the Kibana terminal for "restarting optimizer". Ensure the `--kibana` (or `-k`) flag is present.
...
Thanks to Kibana shared dependency package incremental builds, with subsequent builds only relevant chunks will update which should lead to less Kibana plugins having to be rebuilt.
