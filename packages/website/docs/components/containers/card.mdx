...
If the card links to or references a module that is not GA (beta, lab, etc), pass `betaBadgeProps.children` for the badge text and `betaBadgeTooltipProps.content` (and optionally `betaBadgeTooltipProps.title`) for the tooltip. **EuiCard** positions an **EuiBadge** and wraps it in **EuiToolTip** when tooltip props are provided.
  ...
  <EuiFlexGroup gutterSize="l">
    ...
    <EuiFlexItem>
      <EuiCard
        ...
        title="Monitoring"
        description="Example of a card's description. Stick to one or two sentences."
        betaBadgeProps={{ children: 'Accent', color: 'accent' }}
        betaBadgeTooltipProps={{
          content:
    ...
    <EuiFlexItem>
      <EuiCard
        ...
        title="Lens"
        isDisabled
        description="Disabled cards can have active links using EuiBadge."
        betaBadgeProps={{
        ...
        betaBadgeTooltipProps={{
          content:
            'Disabled cards that have clickable badges will stay hollow.',
        }}
        onClick={() => {}}
      />
    </EuiFlexItem>
