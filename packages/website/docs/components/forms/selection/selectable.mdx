...
Use the `compressed` prop to create a shorter height search input and smaller list items. This is useful when the selectable is used in a space-constrained context, such as a popover, or alongside other compressed form controls for visual consistency.
