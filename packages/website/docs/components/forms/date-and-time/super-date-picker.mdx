...
A zoom in button is also available, but hidden by default. To enable it, pass `showZoomIn: true` in the `showTimeWindowButtons` prop e.g. `showTimeWindowButtons={{ showZoomIn: true }}`.
