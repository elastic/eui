...
If you're using [**EuiPageTemplate**](/docs/templates/page-template), a padding top will be automatically set based on the number of fixed headers on the page. [**EuiFlyouts**](/docs/layout/flyout) will also automatically account for and sit below fixed headers.
...
Use an **EuiHeaderSectionItemButton** to display additional information in [popovers](/docs/layout/popover) or [flyouts](/docs/layout/flyout), such as a user profile or news feed. When using [**EuiFlyout**](/docs/layout/flyout), be sure to wrap it in a [**EuiPortal**](/docs/utilities/portal). When using an [**EuiPopover**](/docs/layout/popover) in conjunction with a **fixed** header, be sure to add the `repositionOnScroll` prop to the popover.

The example below shows how to incorporate **EuiHeaderAlert** components to show a list of updates inside a [**EuiFlyout**](/docs/layout/flyout) and a [**EuiPopover**](/docs/layout/popover) .
