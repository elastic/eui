...
The **EuiCode** and **EuiCodeBlock** components support <Link to="https://prismjs.com/#supported-languages">all language syntaxes</Link> available through the <Link to="https://prismjs.com/">`prismjs` library</Link> and <Link to="https://www.elastic.co/guide/en/kibana/current/esql.html">ES|QL syntax</Link> provided by <Link to="https://github.com/elastic/prismjs-esql">`prismjs-esql` plugin</Link>. The `language` prop can also be omitted to simply render formatted but non-highlighted code.
...
In places like [flyouts](../containers/flyout/index.mdx), you can use `overflowHeight="100%"` to stretch the code block to fill the space. Just be sure that it's parent container is also `height: 100%`.
  ...
  if (isFlyoutVisible) {
    flyout = (
      ...
      >
        ...
        <EuiPanel css={css` block-size: 100%; `} color="transparent" hasBorder={false}>
          <EuiCodeBlock
            language="json"
            overflowHeight={'100%'}
            isCopyable
