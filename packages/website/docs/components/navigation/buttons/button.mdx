...
Button width is generally **determined by content** (label, icon, padding). [Icon-only buttons](#icon) are smaller and **square**. EUI does not set a **default minimum width** but can be applied with the `minWidth` prop. Use `fullWidth` buttons primarily in narrow forms or for mobile responsiveness; use sparingly in larger desktop viewports.
