...
This can be done both on `EuiContextMenu` or a standalone `EuiContextMenuPanel` directly. When `height` is set, the title and any content outside of
the scrollable area remain static while the items are scrollable.
...
export default () => {
  ...
  return (
    <EuiPopover
      ...
      anchorPosition="downLeft"
    >
      <EuiContextMenu initialPanelId={0} panels={panels} height={200} />
    </EuiPopover>
  );
};
```
