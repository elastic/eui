...
const iconComponentDir = path.resolve(srcDir, 'components', 'icon');

const iconDirectories = [
  {
    svgsDir: path.resolve(iconComponentDir, 'svgs'),
    outputDir: path.resolve(iconComponentDir, 'assets'),
    servicesImportPath: '../../../services',
  },
  {
    svgsDir: path.resolve(srcDir, 'components', 'notification_icon', 'svgs'),
...
function compileIcons({ svgsDir, outputDir, servicesImportPath }) {
  const iconFiles = glob.sync('**/*.svg', { cwd: svgsDir, realpath: true });
...
iconDirectories.forEach(compileIcons);
