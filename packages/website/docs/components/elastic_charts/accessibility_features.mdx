...
[Elastic Charts(opens in a new tab or window)](https://elastic.github.io/elastic-charts) provides some accessibilty features for all users.
...
:::note In version 64.1.0, data tables are only available for partition charts.
...
:::note In version 64.1.0, texture fills are only available for XY charts.
...
:::note In version 64.1.0, semantic groupings are only available for goal and gauge charts.
