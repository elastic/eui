---
id: patterns_error_warning
slug: /patterns/error-messages/error-warning
---
# Error warning
...
An unrelated error should be unintrusive, but the user should still be told so they can make an informed decision on whether to continue or not.
...
Do not use an error warning when the error is directly related to the current task. Instead, use the [error page pattern](./error_pages.mdx) or the [error banner pattern](error_banners.mdx/error_).

Do not use an error warning for validating information the user has entered, instead use the [validation errors pattern](./error_validation.mdx).

## How it works

Use the [EuiToast](../../../components/toast#danger) component, with the props:
- `color="danger"`
- `iconType="alert"`
