...
<Example>
  ...
  <Example.Preview>
    Targeting large devices only: {useIsWithinBreakpoints(['l', 'xl']) ? <EuiIcon type="checkCircleFill" color="success" /> : <EuiIcon type="cross" color="danger" />}

    Targeting medium devices and below: {useIsWithinMaxBreakpoint('m') ? <EuiIcon type="checkCircleFill" color="success" /> : <EuiIcon type="cross" color="danger" />}

    Targeting small devices and above: {useIsWithinMinBreakpoint('s') ? <EuiIcon type="checkCircleFill" color="success" /> : <EuiIcon type="cross" color="danger" />}
  </Example.Preview>
  <Example.Snippet>
    ```tsx
