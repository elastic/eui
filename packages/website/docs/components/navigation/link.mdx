...
Like any other [button component](./button/overview.mdx), links can be passed a `color`. Note that the `ghost` type should only be used on dark backgrounds (regardless of theming) as it will always create a white link.
