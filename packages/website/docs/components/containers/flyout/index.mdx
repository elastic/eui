...
EUI provides the `--eui-flyout-offset` CSS variable to allow consumers to declare viewport constraints that affect flyout positioning. This is useful when your application has fixed UI elements (like sidebars or navigation panels) that reduce the effective viewport width.
...
```javascript
document.documentElement.style.setProperty('--eui-flyout-offset', '300px');
```
