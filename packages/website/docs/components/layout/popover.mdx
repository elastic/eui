...
Use the `panelPaddingSize` prop to adjust the padding of the panel content. When using popover titles and footers, this setting will propogate to them. Or you can supply a custom `paddingSize` to either the **EuiPopoverTitle** of **EuiPopoverFooter**.
...
:::warning Removing ownFocus makes it difficult for keyboard-only and screen reader users to navigate to and from your popover.
