...
To increase contrast between the page and the table contents with its navigation
controls, you can enable the panelled layout by setting [`panelled=true`](#EuiInMemoryTable-prop-panelled).
This should be done outside EUI containers like `<EuiPanel>` or `<EuiFlyout>`.

Panelled layout adds a rounded border over the whole table and changes
the background of the pagination bar to `backgroundBaseSubdued`.
...
It is possible to create your own supporting panels for toolbars and other
features by using the `useEuiBasicTablePanelProps` hook. Panels should be passed
to the [`childrenBetween`](#EuiInMemoryTable-prop-childrenBetween) prop.
...
const Panel = () => {
  ...
  return (
    <div {...panelProps} css={[panelProps.css, useEuiPaddingCSS().m]}>
      A custom panel styled with <EuiCode>useEuiBasicTablePanelProps</EuiCode>
    </div>
  );
};
